...
const VERTICAL_ACCEL    = 40
const THROTTLE_RATE     = 12
const YAW_SPEED         = 1.2
const PITCH_SPEED       = 0.8
const ROLL_SPEED        = 0.6
...
export function updateSubship(state: SubshipState, input: RawInput, dt: number): SubshipState {
  ...
  const newRx = Math.abs(input.pitch) > 0.01
    ? Math.max(-1.2, Math.min(1.2, rx + input.pitch * PITCH_SPEED * dt))
    : rx * Math.pow(0.25, dt)
  const newRy = ry + input.yaw * YAW_SPEED * dt
  const newRz = Math.abs(input.roll) > 0.01
    ? Math.max(-1.2, Math.min(1.2, rz + input.roll * ROLL_SPEED * dt))
    : rz * Math.pow(0.15, dt)

  const throttle = input.boost
    ? 5
    : Math.abs(input.throttleDelta) > 0.01
      ? Math.max(-2, Math.min(5, state.throttle + input.throttleDelta * THROTTLE_RATE * dt))
